...
let t1 = Transaction(id: "1", date: Date(), amount: 100, description: "Deposit", isDebit: false)
t1.printDetails()
let t2 = Transaction(id:"2", date: Date(), amount: 30, description: "Chick-fil-A", isDebit: true)
...
AccountDashboard(dataSource: MockAccountDataSource()).showSummary(for: "123")
AccountDashboard(dataSource: LiveAccountDataSource()).showSummary(for: "456")
...
if let z = user.address?.zip {
    print("zip : \(z)")
} else {
    print("No ZIP available")
}
if let z2 = userNoAddress.address?.zip {
    print("ZIP: \(z2)")
} else {
    print("No ZIP available")
}
...
func transfer(from sourceId: String?, to destId: String?, amount: Double?) {
    ...
    let d = destId,
       ...
       a > 0
    {
        print("Transfer $\(String(format: "%.2f", a)) from \(s) to \(d) approved")
    } else {
...
// and what you would do instead.
    // we would never write this because it would crash the whole program since userInputString is rnadom/unknown and we
    // should only use it when nil value is provided by the programmers
    // and I would use a guard let like we did in class to unwrap it safely and provide nil cases to inform the user
    // that it is not a valid URL
...
// TODO 5b: Write a throwing function:
 func executeTransfer(amount: Double, fromBalance: Double, toAccountId: String,
     ...
     guard !toAccountId.isEmpty else { throw TransferError.accountNotFound(id: toAccountId) }
     guard amount <= fromBalance else { throw TransferError.insufficientFunds(available: fromBalance) }
     guard dailyUsed + amount <= dailyLimit else { throw TransferError.dailyLimitExceeded(limit: dailyLimit, attempted: dailyUsed + amount) }
     guard toAccountId != "ERR_NET" else { throw TransferError.networkUnavailable }
     return "Transfer of \(amount) to account \(toAccountId) complete"
 }
//
...
do{
    let result = try executeTransfer(amount: 1500, fromBalance: 2000, toAccountId: "checking", dailyUsed: 4, dailyLimit: 3)
    print(result)
} catch TransferError.invalidAmount {
    print(TransferError.invalidAmount.errorDescription ?? "")
} catch TransferError.insufficientFunds(let avail){
    print("Insufficient funds: \(avail)")
}catch TransferError.dailyLimitExceeded(limit: let limit,let attempted){
    print("Daily limit exceeded: limit: \(limit), attempted: \(attempted)")
} catch TransferError.accountNotFound(id: let id){
    print("Account does not exist \(id)")
} catch TransferError.networkUnavailable{
    print("Network unav ailable")
} catch {
    print("unknown error \(error)")
}

func attempt(_ label: String, _ work: () throws -> String) {
    do {
        print("\(label): \(try work())") }
    catch { print("\(label): \(error as? LocalizedError)?.errorDescription ?? \(error)")
    }
}
...
do{
    let result2 = try executeTransfer(amount: 1500, fromBalance: 2000, toAccountId: "", dailyUsed: 4, dailyLimit: 3)
    print(result2)
} catch TransferError.invalidAmount {
    print(TransferError.invalidAmount.errorDescription ?? "")
} catch TransferError.insufficientFunds(let avail){
...
print(tasks.pop() ?? "empty")
print(tasks.top ?? "empty")
print(tasks.count)
...
print(findLargest([1, 2, 3, 4, 5]) ?? "empty")
print(findLargest(["a", "b", "c"]) ?? "empty")
print(findLargest(["2.14", "3.14","4.21"]) ?? "empty")
print(findLargest([Int]()) ?? "empty")
// ============================================================
