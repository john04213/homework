...
let t1 = Transaction(id: "acc1", date: Date(), amount: 2_500, description: "Direct Deposit", isDebit: false)
print(t1.formattedAmount, t1.description)
var t2 = Transaction(id: "acc2", date: Date(), amount: 45.67, description: "Starbucks", isDebit: true)
...
print("description for t1:\(t1.description)")
print("description for t3:\(t3.description)")
...
print("checking deposit: \(checking.balance)")
checking.withdraw(amount: 200)
print("savings withdraw: \(checking.balance)")
...
//BankAccount is a class so it is a reference type, so checkingRef only copies the reference that it both points to. while Transaction is a struct, which is a value type so t3 copied t1
...
let premAcc =  PremiumBankAccount(id: "acc_003", accountNumber: "1122334455", owner: "Jane Smith", initialBalance: 100.0, overdraftLimit: 500.0)
let pk1 = premAcc.withdraw(amount: 400)
print("withdraw 400: \(pk1)")
premAcc.printSummary()
let pk2 = premAcc.withdraw(amount: 800)
print("withdraw 800: \(pk2)")
premAcc.printSummary()
...
print(describeError(.dailyLimitExceeded(limit: 500)))
