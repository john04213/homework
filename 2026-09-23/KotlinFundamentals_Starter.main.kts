...
fun describe(result: TransferResult): String {
    return when (result) {
        is TransferResult.Success -> "Transfer success, no further actions required"
        is TransferResult.Failure -> "Transfer failed. Please try again later"
    }
}
...
suspend fun fetchAccounts(): List<Account> {
    ...
    val account = mutableListOf(
        Account("Checking", "John", 4444.0),
        Account("Debit", "Joe", 5463.0),
        Account("credit", "Mike", -4333.0)
    )
    return account
}
...
fun main() = runBlocking {
    ...
    val acc = fetchAccounts()
    val negAcc = acc.filter { it.balance < 0}
    val total =  acc.sumOf{it.balance}
    
    println("Overdrawn accounts: ")
    negAcc.forEach { println("${it.name}: ${it.balance.asCurrency()}")}
    println("Total: ${total.asCurrency()}")
    println(describe(TransferResult.Success("Mid-123")))
